...
class FileManager
{
     ...
     * @return array|bool
     */
    public static function upload(string $destination = FileDestinations::COMMON, $file = null, int $validate = self::FILE_TYPE_DOCUMENTS, string $prefix = ''): array
    {
        $response = [
            'status' => false,
        ...
        // Ensure $file is not null and is a valid file object
        if (!empty($file) && $file instanceof UploadedFile && $file->isValid()) {
            ...
            try {
                ...
                // Handle any exception during file upload
                $response['message'] = 'Failed - ' . $e->getMessage();
            }
        } else {
            // Handle invalid file scenario
            $response['message'] = 'Failed - Invalid File';
        }
    ...
    public static function getTemporaryUrl(string $fileName, string $destination = FileDestinations::COMMON): string
    {
        return Storage::temporaryUrl(
            ...
            now()->addMinutes(self::TEMP_UTL_EXPIRY_TIME)
        );
    }
